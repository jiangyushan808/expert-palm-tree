...
<<<<<<< HEAD
float current_angle =0.0f;//剪掉target_ange
=======
...
int16_t    Motor4Speed;
<<<<<<< HEAD
volatile bool yaw_ready = false;       // 表示已收到至少一次合法的 yaw 数据

static bool initial_angle_set = false; // 标志：是否已记录上电第一次角度
static float initial_angle = 0.0f;     // 上电时读取的角度（作为参考）
 float target_angle = 0.0f;      // 目标角度（上电角度或其它）
static float target_angle_zhon = 0.0f;
=======
>>>>>>> 8a274eeea5e1c58bf1cf451062b760e3f76587d0
...
<<<<<<< HEAD
/**
 * @brief 将任意角度归一化到 0~360 度范围内
 * @param a 输入角度（可以是负值或大于360的值）
 * @return 归一化后的角度（0~360）
 */
static float normalize360(float a)
{
    // 使用 fmodf 保证结果在 0..360
...
void CopeSerial1Data(uint8_t ucData)
{
    ...
    static uint8_t sync_state = 0;  // 0:等待帧头, 1:接收数据
    ...
    switch(sync_state) {
        ...
        case 1:  // 接收数据
            // 如果在接收数据过程中又遇到0x55，说明丢包或同步丢失，强制重新同步
            if(ucData == 0x55 && idx != 0) {
                buf[0] = 0x55;
                idx = 1;
                // 保持sync_state=1，继续接收新帧
                break;
            }
            buf[idx++] = ucData;
            if(idx == 11) {
                uint8_t sum = 0;
                if(buf[1] >= 0x51 && buf[1] <= 0x53) {
                    for(uint8_t i = 0; i < 10; i++) sum += buf[i];
                    if(sum == buf[10]) {
                        if(buf[1] == 0x53) {
                            stcAngle.Angle[0] = (int16_t)((uint16_t)buf[2] | ((uint16_t)buf[3] << 8));
                            stcAngle.Angle[1] = (int16_t)((uint16_t)buf[4] | ((uint16_t)buf[5] << 8));
                            stcAngle.Angle[2] = (int16_t)((uint16_t)buf[6] | ((uint16_t)buf[7] << 8));
...
<<<<<<< HEAD
         if (!initial_angle_set && current_yaw)
    {
        // 把第一次读到的 yaw 当做初始角
        initial_angle = current_yaw;

        // 按你的需求选择下面哪一行：
        target_angle_zhon = initial_angle;  // 把目标设为上电时当前角（小车上电即保持此朝向）
         target_angle = 0.0f;        // 或者把目标置为 0°（如果你想把初始角作为 0 的参考点，
                                     // 那么你需要把后续所有角度都用 initial_angle 做偏移）

        initial_angle_set = true;
		}
		current_angle=current_yaw-target_angle_zhon;
//				float Target_yaw=JY61_GetYawCorrected(NO1_yaw);
//				yaw_correction =JY61P_Yaw_correct(cunrent_yaw,Target_yaw);
//				 current_yaw=30;
yaw_correction = JY61P_Yaw_correct(current_angle, target_angle, 0);

//				);
				yaw_correction =Xianfu (yaw_correction ,100);
=======
//				float Target_yaw=JY61_GetYawCorrected(NO1_yaw);
//				yaw_correction =JY61P_Yaw_correct(cunrent_yaw,Target_yaw);
//				 current_yaw=30;
